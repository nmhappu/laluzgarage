...
/// Tight bounding box (viewBox: 223 422 808 418, aspect ratio ~1.933 : 1)
/// Left & right wings inherit primary theme text color, center kinetic spark is #78DF22.
class LaluzLogo extends StatelessWidget {
  final double? width;
  final double? height;
  final double? size;
  final Color? color;
  ...
  const LaluzLogo({
    ...
    this.color,
    this.showGlow = false,
    this.glowColor = const Color.fromRGBO(16, 185, 129, 0.25),
    this.heroTag = 'laluz-brand-logo-hero',
  });
  ...
  Widget build(BuildContext context) {
    ...
    final double resolvedWidth =
        width ?? (height != null ? height! * (808.0 / 418.0) : (size ?? 48));
    final double resolvedHeight =
        height ?? resolvedWidth * (418.0 / 808.0);

    final svgData = '''
<svg viewBox="223 422 808 418" fill="none" xmlns="http://www.w3.org/2000/svg">
  <!-- Left Wing / 'L' Shape -->
  <path
    ...
    final logoWidget = SizedBox(
      ...
      child: Stack(
        ...
        children: [
          if (showGlow)
            Container(
              width: resolvedWidth * 1.25,
              height: resolvedHeight * 1.25,
