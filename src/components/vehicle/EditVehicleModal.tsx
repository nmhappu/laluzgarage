      ...
      >
        ...
        <div className="flex justify-between items-center pl-2 pr-6 sheet-header-safe pb-4 bg-workshop-bg border-b border-workshop-border/30 shrink-0 select-none">
          ...
          <div className="flex-1 pl-2">
            <h2 className="text-lg sm:text-2xl font-black text-secondary tracking-tight uppercase leading-none font-sans">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 max-w-2xl mx-auto w-full">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold uppercase tracking-widest text-workshop-muted px-1">Manufacturer</label>
                <input 
                  required
                  type="text" 
                  value={editingVehicle.make}
                  onChange={e => setEditingVehicle({...editingVehicle, make: e.target.value})}
                  className="w-full bg-workshop-surface border border-workshop-border px-4 py-3 rounded-xl text-sm focus:ring-1 focus:ring-secondary focus:border-secondary outline-none text-workshop-text font-bold transition-all shadow-sm"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold uppercase tracking-widest text-workshop-muted px-1">Model</label>
                <input 
                  required
                  type="text" 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 font-sans">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold uppercase tracking-widest text-workshop-muted px-1">Plate Registration</label>
                <div className="relative">
                  <input 
                    ...
                    className={cn(
                      "w-full bg-workshop-surface border border-workshop-border pl-4 pr-16 py-3 rounded-xl text-sm font-plate font-black focus:ring-1 focus:ring-secondary focus:border-secondary outline-none uppercase tracking-widest transition-all shadow-sm",
                      editingVehicle.plateNumber === "U/R" ? "text-status-urgent bg-workshop-surface/40" : "text-secondary"
                    )}
                    placeholder={editingVehicle.plateNumber === "U/R" ? "UNREGISTERED" : DEFAULT_PLATE_PLACEHOLDER}
                  />
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className={cn(
                      ...
                      editingVehicle.plateNumber === "U/R"
                        ...
                        : "bg-workshop-surface border-workshop-border/30 text-workshop-muted hover:text-status-urgent"
                    )}
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold uppercase tracking-widest text-workshop-muted px-1">Colour</label>
                <input 
                  type="text" 
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold uppercase tracking-widest text-workshop-muted px-1 flex items-center justify-between">
                ...
                <button 
                  ...
                  className="text-[9px] font-black uppercase text-secondary border-b border-secondary/20 hover:opacity-85 transition-all"
                >
              ...
              <input 
                ...
                className="w-full bg-workshop-surface border border-workshop-border px-4 py-3 rounded-xl text-sm font-numeric focus:ring-1 focus:ring-secondary focus:border-secondary outline-none text-workshop-text font-bold transition-all shadow-sm"
              />
            </div>

            <div className="space-y-1.5">
              <label className="text-[10px] font-bold uppercase tracking-widest text-workshop-muted px-1 font-bold">Assigned Owner File</label>
              <select
                value={editingVehicle.customerId}
                onChange={e => setEditingVehicle({ ...editingVehicle, customerId: e.target.value })}
                className="w-full bg-workshop-surface border border-workshop-border px-4 py-3 rounded-xl text-sm outline-none text-workshop-text font-bold focus:ring-1 focus:ring-secondary focus:border-secondary transition-all shadow-sm"
              >
                {customers.map(c => (
