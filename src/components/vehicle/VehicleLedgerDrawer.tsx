      ...
      >
        ...
        <div className="flex justify-between items-center pl-2 pr-6 sheet-header-safe pb-4 bg-workshop-bg shrink-0 select-none">
          ...
          <div className="flex-grow pl-2">
            <h2 className="text-lg sm:text-2xl font-black font-google-sans text-workshop-accent tracking-tight uppercase leading-none">
              {selectedVehicleForLedger.make} {selectedVehicleForLedger.model}
            </h2>
          </div>

          <div className="flex flex-col items-end select-none text-right">
            <span className="text-lg sm:text-2xl font-black font-plate text-secondary uppercase tracking-tight leading-none">
        ...
        <div className="bg-workshop-surface/40 px-6 py-5 border-b border-workshop-border/20 shrink-0">
          <div className="max-w-4xl mx-auto w-full flex items-center justify-between gap-6">
            <div>
              <p className="text-[9px] font-black text-workshop-muted uppercase tracking-widest mb-1 leading-none">Total Maintenance Value</p>
              <p className="text-2xl font-black text-secondary tracking-tighter leading-none">
                {formatCurrency(totalSpend)}
              </p>
            </div>
            <div className="text-right">
              <p className="text-[9px] font-black text-workshop-muted uppercase tracking-widest mb-1 leading-none font-sans">Recorded Jobs</p>
            ...
            {vehicleRecords.length > 0 ? (
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                          <div className="space-y-1.5">
                            <div className="flex items-center gap-3">
                              <span className="text-base font-black text-secondary uppercase tracking-widest leading-none">
                                {formattedDate}
                              </span>
                              <span className={cn(
                          ...
                          <div className="flex flex-col items-start sm:items-end shrink-0 select-none">
                            <p className="text-[8px] font-black text-workshop-muted uppercase tracking-[0.2em] mb-1">Invoiced Amount</p>
                            <p className="text-xl sm:text-2xl font-black text-secondary tracking-tight leading-none font-sans">{formatCurrency(record.totalCost)}</p>
                          </div>
                        </div>

                        <div className="space-y-4">
                          {/* Maintenance Description without box frames, clean border bar */}
                          <div className="pl-4 border-l-2 border-secondary/30 py-1">
                            <p className="text-[9px] font-black text-workshop-muted uppercase tracking-widest mb-2 font-sans">COMPLAINT / REPAIR NOTES</p>
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
                            <div className="border border-workshop-border/30 bg-workshop-surface/25 p-3 rounded-xl flex flex-col justify-between">
                              <span className="text-[8px] font-bold text-workshop-muted uppercase tracking-widest mb-1.5 font-sans">Mileage Recorded</span>
                            ...
                            <div className="border border-workshop-border/30 bg-workshop-surface/25 p-3 rounded-xl flex flex-col justify-between">
                              <span className="text-[8px] font-bold text-workshop-muted uppercase tracking-widest mb-1.5 font-sans">Inventory Consumed</span>
                              <div className="flex items-center gap-1.5">
                            ...
                            <div className="border border-workshop-border/30 bg-workshop-surface/25 p-3 rounded-xl flex flex-col justify-between">
                              <span className="text-[8px] font-bold text-workshop-muted uppercase tracking-widest mb-1.5 font-sans">Labour Charges</span>
                              <span className="text-xs font-black text-workshop-text leading-none mt-1 font-sans">{formatCurrency(record.laborCost)}</span>
                            </div>
                            <div className="border border-workshop-border/30 bg-workshop-surface/25 p-3 rounded-xl flex flex-col justify-between">
                              <span className="text-[8px] font-bold text-workshop-muted uppercase tracking-widest mb-1.5 font-sans">Materials Cost</span>
                          ...
                          {record.partsUsed && record.partsUsed.length > 0 && (
                            <div className="bg-workshop-surface/20 border border-workshop-border/20 rounded-xl p-4 space-y-2.5">
                              <p className="text-[8px] font-black text-workshop-muted uppercase tracking-widest font-sans">Itemized Parts Consumption</p>
                              <div className="space-y-2 font-sans">
                                {record.partsUsed.map((parts, i) => (
                                  <div key={i} className="flex items-center justify-between text-xs px-1 py-0.5 hover:bg-workshop-surface/30 rounded transition-colors font-sans">
                                    ...
                                    <span className="text-secondary font-numeric text-xs font-black">{formatCurrency(parts.unitPrice * parts.quantity)}</span>
                                  </div>
                                ))}
                              </div>
                            </div>
              ...
              <div className="text-center py-24 select-none">
                <div className="w-16 h-16 bg-workshop-surface/50 rounded-2xl flex items-center justify-center mx-auto mb-6 text-workshop-muted border border-workshop-border/20">
                  <ShieldCheck className="w-8 h-8 opacity-30 text-secondary" />
                </div>
        ...
        <div className="pt-4 px-6 sheet-footer-safe bg-workshop-bg border-t border-workshop-border/30 flex justify-end shrink-0">
          <button 
            type="button"
            onClick={onClose}
            className="px-8 py-3.5 bg-secondary text-white text-xs font-black uppercase tracking-widest rounded-xl hover:brightness-110 shadow-lg shadow-secondary/20 active:scale-95 transition-all outline-none"
          >
            Close Ledger
          </button>
        </div>
